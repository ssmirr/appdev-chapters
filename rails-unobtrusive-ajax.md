...
This dramatically increases cost and reduces development velocity, but in some cases, we have no choice. But only use this approach when you don't have a choice! Far too many teams choose it when their app really doesn't need it, or they choose it too soon. But when we do decide to go down this road, the nice part is that the same robust JSON API that we develop can feed native iOS and Android clients.
