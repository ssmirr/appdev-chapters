...
For cases like that, we will have to consider breaking apart our application into a back-end that serves only JSON (i.e. doesn't support requests of format HTML nor JS); and front-end clients will have to consume the API. Typically each will be developed by their own specialized team, since now both the back-end and front-end require much more work. We could build a complicated web-client ourselves using `$().ajax`, but it's usually better to use a framework like React, Vue, or Angular.

This dramatically increases cost and reduces development velocity, but in some cases, we have no choice. But only use this approach when you don't have a choice! Far too many teams choose it when their app really doesn't need it, or they choose it too soon. But if and when we do decide to go down this road, the nice part is that the same robust JSON API that we develop can feed native iOS and Android clients.
